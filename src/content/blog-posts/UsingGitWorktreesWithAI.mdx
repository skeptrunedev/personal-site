---
title: "Using Git Worktrees for Massively Parallel AI SWE Agents"
slug: git-worktrees-with-AI
summary: >-
  I spent months chasing developer superpowers with AI codegen and struggled to feel any productivity gain, then immediately found it when I started using git worktrees to run multiple instances of terminal-driven coding agents like Claude Code and Codex in parallel.
...
### Making the case for parallel codegen

I like to think of the parallel inference process like [hiring multiple freelancers on Fiverr for the same task](https://x.garrytan/status/1918027171039461435). Each freelancer works in their own environment and comes up with a unique solution to the problem. This way, you can compare outputs and select the best one, rather than relying on a single person who may not fully understand your needs or deliver subpar results.

This spray and pray approach is the most effective way to leverage codegen, but hard to achieve with standard git in a single directory. Multiple agents running in one folder can overwrite each other's changes, causing confusion. [Git worktrees](https://git-scm.com/docs/git-worktrees) solve this by creating isolated working directories that share the same repository. Each worktree can have its own branch and agent, allowing you to run multiple simultaneously without them crossing wires.

This approach is more complex and token-intensive than using a single agent, but provides significant quality benefits. These coding agent systems are stochastic, so running the same prompt multiple times produces different outputs. By running several in parallel with identical prompts, you create a [Monte Carlo process](https://en.wikipedia.org/wiki/Monte_Carlo_method) that lets you compare multiple solutions and choose the best one, rather than risking getting only a single potentially suboptimal result.
...
However, it's early days for this approach and not much tooling exists yet.

### Current state of the art: manual worktrees and tmux

I've been using git worktrees with AI agents for a while now, and while it has significantly improved my productivity, the process is full of papercuts.
