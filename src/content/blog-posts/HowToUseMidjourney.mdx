---
title: "How to Use Midjourney - Web Dev's Starter Guide"
...
I was initially going to use these for [uzi.sh](https://uzi.sh) which is a tool for parallel LLM coding agents, but ended up going with a different final image since I only needed. However, given that, I wanted a red color scheme to inspire action and speed, so went that direction with the images below.
...
My neutral prompt here was `Portrait photography of a woman, glow behind, futuristic vibe, flash photography, color film, analog style, imperfect --ar 3:4 --v 7`. Basically anything which describes a general subject and characteristics without specificity on style, camera angle, action, or other details will work.
...
You first need to add a svg filter definition to your HTML file such that the CSS can reference it later to put the grain on top of the background image. The filter uses `feTurbulence` to create a fractal noise pattern, and `feColorMatrix` to adjust the opacity.
...
Use what you see from others to inspire your own work, but don't just copy and paste. Use AI to create get the unique style in your head out onto the screen. Develop your own voice and vision, and let AI help you express it.
